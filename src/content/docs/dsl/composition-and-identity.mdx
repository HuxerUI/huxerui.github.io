...
```cpp fragment
View StatusBadge(std::string_view label, Color color) {
  return Text(label, TextRole::Label)
      .With(
...
A stateless function does not need `[[huxerui::composable]]`.
...
```cpp fragment
[[huxerui::composable]]
View ExpandableSection(std::string_view title, View content) {
  auto expanded = UseState(false);

  return Column {
    Button(title).OnClick([expanded] {
      expanded = !expanded;
    }),
